...
Finally let's try to create a composite figure including the PCA and heatmap because there are related patterns seen in both figures that we would like to make clear.
...
pca_data3 <- pca_data2 %>% 
  ...
  mutate(colgroup = ifelse(name %in% hm_col_groups$up,"up","neutral")) %>% 
  mutate(colgroup = ifelse(name %in% hm_col_groups$down,"down",colgroup))

colgroup_colours <- diverging_hcl(3, palette = "Blue-Red 2")

pca_plot <- pca_data3 %>% 
  ggplot(aes(y=PC1,x=PC2)) + 
  geom_point(aes(shape=HardCoral),size=4) + 
#  geom_point(aes(x=-1,y=PC2, color=colgroup),size=4)+
  geom_label_repel(aes(label=name, fill=colgroup))  + 
  ylab(paste0("PC1: ",percentVar2[1],"% variance")) + 
  xlab(paste0("PC2: ",percentVar2[2],"% variance"))  +  
  scale_fill_discrete_diverging(palette = "Blue-Red 2") +
...
hm_plot <- grid.grabExpr(draw(Heatmap(ct_heatmap_data_relative, 
                                      col = col_fun,
...
plot_grid(pca_plot,hm_plot,ncol = 1, 
          rel_heights = c(0.4,0.6), 
          labels = c("A","B"),
          label_x = 0, label_y = 0,
          hjust = -0.5, vjust = -0.5)

ggsave2("figures/pca_hm.png", width = 12,height = 7)
```
