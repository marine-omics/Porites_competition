...
row_anno_data <- readxl::read_excel("raw_data/annotated_DEG_hm_ira.xlsx",na = "NA") %>% 
  ...
  mutate(GPCR = ifelse(!is.na(GPCR),"G","FALSE")) %>% 
      select(cluster_id,Secreted,Immunity,Stress,CellCell,Apoptosis,Toxin,GPCR) %>% as.data.frame()

col_anno_data <- data.frame(Mesenteries = colnames(ct_heatmap_data_relative) %in% mesenteries_groups)



row_anno_data <- row_anno_data[match(rownames(ct_heatmap_data_relative),row_anno_data$cluster_id),] %>% 
  select(-cluster_id)
```
...
row_anno_l <- HeatmapAnnotation(info = as.matrix(row_anno_data),which = "row", 
