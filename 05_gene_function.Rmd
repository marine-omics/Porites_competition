...
automatic_annotations <- read_rds("raw_data/annotated_clusters_porites.rda") %>% 
  select(cluster_id,transcript_id,RNAMMER,Pfam,SignalP,TmHMM,eggnog,Kegg,ipro_pfam,ipro_pfam_desc,ipr_go,best_swissprot,Entry,Protein_names,Gene_names,Organism,Keywords,swissprot_go,plutea_id)
...
```{r}
# Join with old cluster data
annotated_DEG_barnacles <- readxl::read_excel("raw_data/annotated_DEG.xlsx")

annotated_DEG_hm_ira <- annotated_DEG_hm %>% select(cluster_id, hm_cluster) %>% left_join(annotated_DEG_barnacles,by="cluster_id")

write_tsv(annotated_DEG_hm_ira,"raw_data/annotated_DEG_hm_ira.tsv")
```
