...
Load differentially expressed genes between HardCoral treatments obtain data from analysis with DESeq2 (02_deseq). 
...
# This is temporary to remove columns that will be cleanup in the final version of the manual annotation database
colnames_to_remove <- unique(c(colnames(ct_res_vsd2),colnames(P.cylindrica_144087anno)))[-1]

Manual <- read.csv("raw_data/DE_174_database_underEdition2020.csv") %>% 
...
```{r, gene function }
#DE_174_database<- read.csv('raw_data//DE_174_database_underEdition2020.csv')
DE_174_database <- DEG_174 %>% as.data.frame()
...
upset(DE_174_database, sets= c("Immunity", "Sensory.Nervous", "Response","CELLULAR.STRESS", "Stimulus"), 
...
#CELLULAR.STRESS
upset(DE_174_database, sets= c( "Ubiquitination","Mucus","Antioxidants", "Immunity", "CELLULAR.STRESS"), 
         order.by= c('freq','degree'), 
         keep.order= TRUE,
         sets.bar.color("orange"), 
         point.size=2,
...
#IMMUNE RESPONSE TO COMPETITION
upset(DE_174_database, sets= c("Immunity", "Apoptosis",'Immune.activation',"Pro.apoptotic","Inflamation", "CELLULAR.STRESS" ), 
         order.by= c('freq','degree'), 
         keep.order= TRUE,
         sets.bar.color("orange"), 
         point.size=2,
         ...
         text.scale =1)
#BEHAVIOUR
upset(DE_174_database, sets= c('GPCRs',"Food.related","Autophagy","TissueRemodelling_Gowth","Sensory.Nervous"), 
      order.by= c('freq','degree'), 
      keep.order= TRUE,
      sets.bar.color("orange"), 
      point.size=2,
