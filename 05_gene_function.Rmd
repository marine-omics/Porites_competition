...
fanno_data <- row_anno_data %>% 
  ...
  filter(value!=FALSE) %>% 
  filter(name %in% c("Secreted","Immunity","CellCell", "Stress"))

fanno_data$name <- factor(fanno_data$name,levels=c("Secreted","Stress","Immunity","CellCell"))

fanno_data %>% 
  filter(name!="Secreted") %>% 
